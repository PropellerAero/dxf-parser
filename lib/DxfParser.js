...
DxfParser.prototype.parseStream = function (stream, done) {
	...
	function onData(chunk) {
		...
		if (++i % 10 === 0) {
			console.log(
				`chunk ${i} lines: ${dxfLines.length} string length: ${dxfString.length}`
			);
		}
		if (STREAM_THE_STREAM) {
			var chunkLines = dxfString.split(LINE_SPLIT_REGEX);
			if (chunkLines.length > 1) {
				dxfString = chunkLines.pop();
				dxfLines.push(...chunkLines);
			}
		}
	}
	...
	function onEnd() {
		try {
			if (STREAM_THE_STREAM) {
				if (dxfString.length > 0) {
					dxfLines.push(dxfString);
				}

				var dxf = self._parse(dxfLines);
			} else {
				var dxf = self._parse(dxfString.split(LINE_SPLIT_REGEX));
			}
		} catch (err) {
			return done(err);
		}
...
DxfParser.prototype.parseStream2 = function (stream, done) {
